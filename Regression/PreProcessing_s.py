...
def ready_data(data, test=False):
    ...
    data = feature_encoder(data, encode_cols, test=test)
    data = fill_nulls(data)
    ...
    data = feature_scaling2(data, scale_cols, 0, 1)
    data['T5'] = data['T5'].fillna(data['T5'].mean())

    # dropping columns
    cols = ['Tags', 'Reviewer_Score', 'Review_Date']
...
def feature_encoder(x, cols, test=False):
    ...
    for c in cols:
        try:
            ...
            if not test:
                ...
                pickle._dump(lbl, open(f"Encoding{i}.pkl",'wb'))
            else:
                lbl = pickle.load(open(f"Encoding{i}.pkl",'rb'))
